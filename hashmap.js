...
var linkedList_1 = require("./ts/linkedList");
function HashMap(loadFactor) {
	...
	function set(key, value) {
		...
		var idxInBucket =
			(_a = keyBuckets[idx]) === null || _a === void 0
				? void 0
				: _a.find(key);
	...
	function grow() {
		...
		clear();
		for (
			var _i = 0, currentEntries_1 = currentEntries;
			_i < currentEntries_1.length;
			_i++
		) {
			var _a = currentEntries_1[_i],
				key = _a[0],
				value = _a[1];
			set(key, value);
		}
	...
	function shrink() {
		...
		clear();
		for (
			var _i = 0, currentEntries_2 = currentEntries;
			_i < currentEntries_2.length;
			_i++
		) {
			var _a = currentEntries_2[_i],
				key = _a[0],
				value = _a[1];
			set(key, value);
		}
	...
	function length() {
		var len = 0;
		for (var i = 0; i < capacity; i++) len += keyBuckets[i].size();
		return len;
	}
	function clear() {
		keyBuckets = [];
	...
	function keys() {
		...
		for (var i = 0; i < capacity; i++) {
			var curNode =
				(_a = keyBuckets[i]) === null || _a === void 0
					? void 0
					: _a.headNode();
			while (curNode) {
	...
	function values() {
		...
		for (var i = 0; i < capacity; i++) {
			var curNode =
				(_a = valueBuckets[i]) === null || _a === void 0
					? void 0
					: _a.headNode();
			while (curNode) {
		...
		for (var i = 0; i < capacity; i++) {
			var keyNode =
				(_a = keyBuckets[i]) === null || _a === void 0
					...
					: _a.headNode();
			var valNode =
				(_b = valueBuckets[i]) === null || _b === void 0
					? void 0
					: _b.headNode();
